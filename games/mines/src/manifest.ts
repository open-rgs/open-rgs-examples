// mines — manifest builder. Complex round (open/step/close), so it loads the
// `json` Lua extension for state serialization and declares a settle-at-current
// autoclose so an abandoned round banks its safe tiles instead of vanishing.
...
export async function buildManifest(opts: BuildOptions = {}): Promise<GameManifest> {
  const math = await loadLuaMath(resolve(here, "../maths/mines.lua"), {
  ...
  return defineGame({
    ...
    modes: {
      default: { math, stakeMultiplier: 1, label: "Mines" },
    },
  });
}
