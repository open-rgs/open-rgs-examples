// chicken-road — manifest builder. Complex round; json prelude + autoclose.
...
export async function buildManifest(opts: BuildOptions = {}): Promise<GameManifest> {
  const math = await loadLuaMath(resolve(here, "../maths/chicken-road.lua"), {
  ...
  return defineGame({
    ...
    modes: {
      default: { math, stakeMultiplier: 1, label: "Chicken Road" },
    },
  });
}
